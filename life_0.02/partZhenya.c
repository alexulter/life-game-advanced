...
#include "shared.h"
     ...
     int edge_condition_t (struct field **space_array  )
 	   {   
   		 int i,j;
      			 for (i = 1; i < vert_size; i++)
  			        {
          				  space_array[i][1].temperature= (TEMP_MAX-TEMP_MIN)*frand()+TEMP_MIN;
				          space_array[i][hor_size-1].temperature=(TEMP_MAX-TEMP_MIN)*frand()+TEMP_MIN;
  			        }
  				     for (j = 1; j < hor_size; j++)
   				        {
					           space_array[1][j].temperature= (TEMP_MAX-TEMP_MIN)*frand()+TEMP_MIN;
					           space_array[vert_size-1][j].temperature=(TEMP_MAX-TEMP_MIN)*frand()+TEMP_MIN;
				        }
     ...
     int edge_condition_o (struct field **space_array  )
	    ...
	    int i,j;
      		 for (i = 1; i < vert_size; i++)
       		   {
       			     space_array[i][1].oxygen= (OXY_MAX)*frand();
			     space_array[i][hor_size-1].oxygen=(OXY_MAX)*frand();
	           }
      			 for (j = 1; j<hor_size; j++)
     			 	   {
 				          space_array[1][j].oxygen= (OXY_MAX)*frand();
 				          space_array[vert_size-1][j].oxygen=(OXY_MAX)*frand();
  			           }
