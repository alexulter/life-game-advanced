...
static gint abc = 0;
static gint LRUN = 0;
// рисует  один чёрный прямоугольник
static void 
...
/*static gboolean temp (GtkWidget *da, struct field **space_array)
{
...
/*shows field dependindg on what is needed to show*/
 static void  show_field( GtkWidget *da)
{	
	gint i,j;
	GdkGC *gc; 
	GdkColor color;
...
g_signal_connect (G_OBJECT (window), "destroy",
 ...
 gtk_widget_set_size_request (da, (hor_size-4)*SIZE_OF_CELL, (vert_size-4)*SIZE_OF_CELL);
 ...
 button_o = gtk_button_new_with_label ("Show oxygen");
	...
	g_signal_connect (G_OBJECT(button_quit),
			...
			G_CALLBACK(to_exit),
			NULL);
	g_signal_connect (G_OBJECT(button_step), "clicked", G_CALLBACK(life), space_array);
	...
	g_signal_connect (G_OBJECT(da),
			...
			NULL);
	g_signal_connect (G_OBJECT(button_run), "clicked", G_CALLBACK(life_run), space_array);
	g_signal_connect (G_OBJECT(button_stop), "clicked", G_CALLBACK(life_stop), space_array);
//	g_signal_connect (G_OBJECT(button_button_t), "clicked", G_CALLBACK(life_run), space_array);
